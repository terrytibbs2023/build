from tmdbbingiehelper.lib.addon.consts import TMDB_PARAMS_EPISODES, TMDB_PARAMS_SEASONS, TMDB_ALL_ITEMS_LISTS
...
def get_season_list(self, tmdb_id, special_folders=0, get_detailed=False):
    ...
    from tmdbbingiehelper.lib.addon.plugin import ADDONPATH, get_localized
...
def get_cast_list(
    ...
    for key in keys:
        if key == 'crew':
            crew = response.get('crew') or []
            for c in crew:
                c['__priority'] = DEPARTMENT_PRIORITY.get(c.get('department'), 999)
            cast_list += sorted(crew, key=lambda x: (x['__priority'], x.get('job', '').lower()))
        else:
...
def get_daily_list(export_list, sorting=None, reverse=False):
    ...
    from tmdbbingiehelper.lib.files.futils import use_json_filecache
...
def get_all_items_list(tmdb_type, page=None):
    ...
    from bingie.parser import try_int
    from tmdbbingiehelper.lib.addon.plugin import convert_type
