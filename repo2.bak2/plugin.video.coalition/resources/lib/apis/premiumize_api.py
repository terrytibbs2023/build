...
class PremiumizeAPI:
	def __init__(self):
		...
		self.user_agent = 'coalition for Kodi'
	...
	def auth(self):
		...
		data = {'code': device_code, 'grant_type': 'device_code', 'client_id': self.client_id}
		try:
			qr_url = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1&data='
			qr_icon = qr_url + quote_plus(response['verification_uri'])
		...
		kodi_utils.progressDialog.update(100,
			line % (ls(32517), ls(32700) % response.get('verification_uri'), ls(32701) % response.get('user_code'))
		)
	...
	def add_uncached_torrent(self, magnet_url, pack=False):
		...
		def _return_failed(message=32574, cancelled=False):
			...
			if cancelled:
				if kodi_utils.confirm_dialog(heading=32733, text=32044, top_space=True): kodi_utils.ok_dialog(heading=32733, text=ls(32732) % ls(32061))
	...
	def user_cloud(self, folder_id=None):
		if folder_id:
			string = 'coalition_pm_user_cloud_%s' % folder_id
			url = 'folder/list?id=%s' % folder_id
		else:
			string = 'coalition_pm_user_cloud_root'
			url = 'folder/list'
		return cache_object(self._get, string, url, False, 0.5)

	def user_cloud_all(self):
		string = 'coalition_pm_user_cloud_all_files'
		url = 'item/listall'
		return cache_object(self._get, string, url, False, 0.5)
	...
	def get_item_details(self, item_id):
		string = 'coalition_pm_item_details_%s' % item_id
		url = 'item/details'
		data = {'id': item_id}
		args = [url, data]
	...
	def get_hosts(self):
		string = 'coalition_pm_valid_hosts'
		url = 'services/list'
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT id FROM maincache WHERE id LIKE ?""", ('coalition_pm_user_cloud%',))
				try:
			...
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('coalition_pm_transfers_list',))
				clear_property("coalition_pm_transfers_list")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('coalition_pm_valid_hosts',))
				clear_property('coalition_pm_valid_hosts')
				dbcon.commit()
				dbcon.close()
