...
from modules.utils import sort_list, sort_for_article, make_thread_list, jsondate_to_datetime, paginate_list
...
session.mount('https://api.trakt.tv', requests.adapters.HTTPAdapter(pool_maxsize=100))

def call_trakt(path, params={}, data=None, with_auth=True, method=None, pagination=False, page=1):
	def send_query():
		if with_auth is True:
			token = get_setting('trakt.token')
			if token: headers.update({'Authorization': 'Bearer %s' % token})
		if data is not None:
			assert not params
			resp = session.post(API_ENDPOINT % path, data=json.dumps(data), headers=headers, timeout=timeout)
		elif method == 'post': resp = session.post(API_ENDPOINT % path, headers=headers, timeout=timeout)
		...
		except Exception as e: logger('trakt error', str(e))
		return resp
	headers = {'Content-Type': 'application/json', 'trakt-api-version': '2', 'trakt-api-key': CLIENT_ID}
	if pagination: params.update({'page': page})
	response = send_query()
	if response.status_code in (403, 429, 502, 503, 504, 520, 521, 522):
		if 'Retry-After' in response.headers: kodi_utils.sleep(1000 * response.headers['Retry-After'])
		if response.status_code == 403: trakt_refresh()
		response = send_query()
...
def trakt_auth(dummy):
	from urllib.parse import quote_plus
	token = ''
	line = '%s[CR]%s[CR]%s'
	headers = {'Content-Type': 'application/json', 'trakt-api-version': '2', 'trakt-api-key': CLIENT_ID}
	code = {'client_id': CLIENT_ID}
	response = session.post(API_ENDPOINT % 'oauth/device/code', data=json.dumps(code), headers=headers, timeout=timeout).json()
	device_code = response['device_code']
	expires_in = int(response['expires_in'])
	sleep_interval = int(response['interval'])
	data = {'code': device_code, 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
	try:
		qr_url = 'https://api.qrserver.com/v1/create-qr-code/?size=256x256&qzone=1&color=f00&data='
		qr_icon = qr_url + quote_plus(response['verification_url'])
	...
	kodi_utils.progressDialog.update(100,
		line % ('Authorize Trakt Service', ls(32700) % response.get('verification_url'), ls(32701) % response.get('user_code'))
	)
	...
	while not kodi_utils.progressDialog.iscanceled() and time_passed < expires_in:
		...
		progress = 100 - int(100 * time_passed/expires_in)
	...
	if token:
		...
		response = session.get(API_ENDPOINT % '/users/me', headers=headers, timeout=timeout).json()
		...
		while kodi_utils.get_visibility('Window.IsVisible(addonsettings)'): kodi_utils.sleep(100)
...
def trakt_progress(action, media, media_id, percent, season=None, episode=None, resume_id=None, refresh_trakt=False):
	if action == 'clear_progress':
		...
		result = call_trakt(url, method='delete') # result = call_trakt(url, is_delete=True)
	else:
		url = 'scrobble/pause'
...
def trakt_collection_lists(media_type, param1, param2):
	...
	window_property_name = 'coalition_trakt_collection_%s' % string_insert
...
def make_new_trakt_list(params):
	...
	mode = params['mode']
	list_title = kodi_utils.dialog.input('coalition')
...
def trakt_account_info_dialog(dummy):
	from datetime import timedelta
	try:
		kodi_utils.show_busy_dialog()
		account_info = call_trakt('users/settings', with_auth=True)
		stats = call_trakt('users/%s/stats' % account_info['user']['ids']['slug'], with_auth=True)
		username = account_info['user']['username']
		timezone = account_info['account']['timezone']
		joined = js2date(account_info['user']['joined_at'], '%Y-%m-%dT%H:%M:%S.%fZ')
...
def trakt_revoke(dummy):
	if not kodi_utils.confirm_dialog(): return
	data = {'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET, 'token': get_setting('trakt.token')}
