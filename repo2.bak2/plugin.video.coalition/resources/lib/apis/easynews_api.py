...
class EasyNewsAPI:
	...
	def search(self, query, expiration=48):
		...
		string = 'coalition_EASYNEWS_SEARCH_' + urlencode(self.params)
		...
		def _process():
			for item in files:
				try:
					...
					from modules.kodi_utils import logger
					logger('coalition easynews API Exception', str(e))
...
def clear_media_results_database():
	...
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'coalition_EASYNEWS_SEARCH_%'")
	easynews_results = [str(i[0]) for i in dbcur.fetchall()]
	if not easynews_results: return 'success'
	try:
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'coalition_EASYNEWS_SEARCH_%'")
