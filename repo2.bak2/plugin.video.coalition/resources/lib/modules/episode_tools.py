...
from indexers.watched import get_next_episodes, get_watched_status_tvshow, get_watched_info_tv
...
poster_empty = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/media/box_office.png')
fanart_empty = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/fanart.png')
string = str
	...
	def build_content(item):
		try:
			...
			if KODI_VERSION < 20:
				...
				videoinfo.setDirectors(meta.get('director').split(', '))
				videoinfo.setDuration(meta.get('duration'))
				videoinfo.setFirstAired(meta_get('premiered'))
				videoinfo.setGenres(meta.get('genre').split(', '))
				videoinfo.setIMDBNumber(imdb_id)
				videoinfo.setMediaType('episode')
				videoinfo.setMpaa(meta.get('mpaa'))
				videoinfo.setPlaycount(playcount)
				videoinfo.setPlot(meta_get('plot'))
				videoinfo.setRating(meta_get('rating'))
				videoinfo.setStudios(meta_get('studio').split(', '))
				videoinfo.setTitle(meta_get('title'))
				videoinfo.setTrailer(meta_get('trailer'))
				videoinfo.setTvShowStatus(meta_get('status'))
				videoinfo.setTvShowTitle(meta_get('tvshowtitle'))
				videoinfo.setVotes(meta_get('votes'))
				videoinfo.setWriters(meta_get('writer').split(', '))
				videoinfo.setYear(int(meta_get('year')))
...
def execute_nextep(meta, nextep_settings):
	...
	def _get_nextep_url():
		...
		return kodi_utils.get_property('coalition_background_url')
	def _confirm_threshold():
		nextep_threshold = nextep_settings['threshold']
		if nextep_threshold == 0: return True
		try: current_number = int(kodi_utils.get_property('coalition_total_autoplays'))
		except: current_number = 1
		if current_number == nextep_threshold:
			current_number = 1
			kodi_utils.set_property('coalition_total_autoplays', str(current_number))
	...
	def _control():
		...
		return final_action
	kodi_utils.clear_property('coalition_background_url')
	player = coalitionPlayer()
...
def get_random_episode(tmdb_id, continual=False):
	...
	if continual:
		episode_list = []
		try:
			episode_history = json.loads(kodi_utils.get_property('coalition_random_episode_history'))
			if tmdb_key in episode_history: episode_list = episode_history[tmdb_key]
			else: kodi_utils.set_property('coalition_random_episode_history', '')
		except: pass
		first_run = len(episode_list) == 0
		episodes_data = [i for i in episodes_data if not i in episode_list]
		if not episodes_data:
			kodi_utils.set_property('coalition_random_episode_history', '')
	...
	if continual:
		...
		kodi_utils.set_property('coalition_random_episode_history', json.dumps(episode_history))
...
def play_random_continual(tmdb_id):
	...
	url = kodi_utils.get_property('coalition_background_url')
	kodi_utils.clear_property('coalition_background_url')
	while player.isPlayingVideo(): kodi_utils.sleep(100)
	player.run(url)
