...
class MenuEditor:
	def __init__(self, params):
		self.params = params
		self.menu_item = dict(parse_qsl(kodi_utils.get_infolabel("ListItem.FileNameAndPath").replace('plugin://plugin.video.coalition/?','')))
	...
	def _menu_select(self, choice_items, menu_name, heading='coalition', multi_line='false', position_list=False):
		def _builder():
	...
	def _get_external_name_input(self, current_name):
		new_name = kodi_utils.dialog.input('coalition', defaultt=current_name)
	...
	def shortcut_folder_make(self):
		list_name = kodi_utils.dialog.input('coalition')
