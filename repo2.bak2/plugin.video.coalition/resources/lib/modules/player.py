...
from indexers import watched as ws
...
class coalitionPlayer(kodi_utils.xbmc_player):
	def __init__ (self):
	...
	def run(self, url=None, media_type=None):
		...
		try:
			...
			self.meta = json.loads(kodi_utils.get_property('coalition_playback_meta'))
			...
			else: bookmark = self.bookmarkcoalition()
			...
			try:
				...
				if self.media_type == 'movie':
					if KODI_VERSION < 20:
						...
						videoinfo.setCountries(self.meta_get('country'))
						videoinfo.setDirectors(self.meta_get('director').split(', '))
						videoinfo.setDuration(duration)
						videoinfo.setGenres(genre.split(', '))
						videoinfo.setIMDBNumber(self.imdb_id)
						videoinfo.setMediaType('movie')
						videoinfo.setMpaa(self.meta_get('mpaa'))
						videoinfo.setPlot(plot)
						videoinfo.setPremiered(premiered)
						videoinfo.setRating(rating)
						videoinfo.setStudios((studio,))
						videoinfo.setTagLine(self.meta_get('tagline'))
						videoinfo.setTitle(self.title)
						videoinfo.setTrailer(trailer)
						videoinfo.setVotes(votes)
						videoinfo.setWriters(self.meta_get('writer').split(', '))
						videoinfo.setYear(int(self.year))
				else:
					if KODI_VERSION < 20:
						listitem.setCast(self.meta_get('cast', []))
						...
						videoinfo.setDirectors(self.meta_get('director').split(', '))
						videoinfo.setDuration(duration)
						videoinfo.setEpisode(self.episode)
						...
						videoinfo.setPlot(plot)
						videoinfo.setRating(rating)
						videoinfo.setSeason(self.season)
						videoinfo.setStudios((studio,))
						videoinfo.setTitle(self.meta_get('ep_name'))
						videoinfo.setTrailer(trailer)
						videoinfo.setTvShowStatus(self.meta_get('status'))
						videoinfo.setTvShowTitle(self.title)
						videoinfo.setVotes(votes)
						videoinfo.setWriters(self.meta_get('writer').split(', '))
						videoinfo.setYear(int(self.year))
	...
	def media_watched_marker(self):
		...
		try:
			if self.current_point >= self.set_watched:
				...
				kodi_utils.clear_property('coalition_nextep_autoplays')
				kodi_utils.clear_property('coalition_random_episode_history')
...
class Subtitles(kodi_utils.xbmc_player):
	...
	def get(self, query, imdb_id, season, episode):
		...
		sub_filename = 'coalitionSubs_%s_%s_%s' % (imdb_id, season, episode) if season else 'coalitionSubs_%s' % imdb_id
