...
from gears.control import getSettingDefault as gears_default_settings, setting as gears_getSetting, setSetting as gears_setSetting
...
string = str
#source_folder_location = 'special://home/addons/script.module.gearsscrapers/lib/gearsscrapers/sources_gearsscrapers/%s'
source_folder_location = 'special://home/addons/plugin.video.coalition/resources/lib/gears/sourcesdir/%s'
...
def internal_sources(active_sources, prescrape=False):
	def import_info():
		files = kodi_utils.list_dirs(kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/lib/scrapers'))[1]
		for item in files:
			try:
...
def toggle_all(folder, setting, silent=False):
	try:
		...
		for i in sourcelist:
			...
			gears_setSetting(source_setting, setting)
		if silent: return
...
def enable_disable(folder):
	try:
#		icon = kodi_utils.translate_path('special://home/addons/script.module.gearsscrapers/icon.png')
		icon = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/lib/gears/media/icon.png')
		...
		for i in all_sources:
			if i in chosen: gears_setSetting('provider.' + i, 'true')
			else: gears_setSetting('provider.' + i, 'false')
		return _ext_scrapers_notice(32576)
...
def set_default_scrapers():
	...
	for i in all_scrapers:
		scraper = 'provider.' + i
		default_setting = gears_default_settings(scraper)
		gears_setSetting(scraper, default_setting)

def scrapers_status(folder='all'):
	providers = scraper_names(folder)
	enabled = [i for i in providers if gears_getSetting('provider.' + i) == 'true']
