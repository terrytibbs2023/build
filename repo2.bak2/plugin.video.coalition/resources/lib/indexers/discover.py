...
icon_directory = 'special://home/addons/plugin.video.coalition/resources/media/%s'
default_icon = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/media/discover.png')
...
class Discover:
	def __init__(self, params):
		...
		if self.media_type: self.window_id = 'coalition_%s_discover_params' % self.media_type.upper()
		else: self.window_id = ''
	...
	def similar_recommended(self):
		...
		for item in results:
			...
			else: icon = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/media/box_office.png')
	...
	def cast(self):
		...
		if len(result) > 1:
			for item in result:
				...
				else: icon = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/media/people.png')
	...
	def history(self, media_type=None, display=True):
		...
		string = 'coalition_discover_%s_%%' % media_type
	...
	def help(self):
#		text_file = kodi_utils.translate_path('special://home/addons/plugin.video.coalition/resources/coalition_discover.txt')
#		return kodi_utils.show_text(self.heading_base % ls(32487), file=text_file)
		return kodi_utils.show_text(self.heading_base % ls(32487), meta_lists.discover_help)
	...
	def _add_defaults(self):
		...
		self._add_dir({'mode': 'discover.export', 'media_type': self.media_type, 'list_name': ls(32667) % name}, icon=kodi_utils.translate_path(icon_directory % 'item_next.png'))
	...
	def _build_url(self, query):
		return ''.join(['plugin://plugin.video.coalition/?', urlencode(query)])
...
def set_history(media_type, name, query):
	...
	string = 'coalition_discover_%s_%s' % (media_type, query)
