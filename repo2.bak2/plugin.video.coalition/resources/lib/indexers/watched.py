...
def clear_local_bookmarks():
	try:
		...
		dbcur = set_PRAGMAS(dbcon)
		file_ids = dbcur.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.coalition%'").fetchall()
...
def set_bookmark_kodi_library(media_type, tmdb_id, curr_time, total_time, season='', episode=''):
	...
	try:
		info = metadata.movie_meta('tmdb_id', tmdb_id, meta_user_info, get_datetime()) if media_type == 'movie' else metadata.tvshow_meta('tmdb_id', tmdb_id, meta_user_info, get_datetime())
