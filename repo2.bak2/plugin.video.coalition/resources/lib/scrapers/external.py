...
class source:
	def __init__(self, source_dict, debrid_torrents, debrid_hosters, internal_scrapers, prescrape_sources, display_uncached_torrents, progress_dialog, disabled_ignored=False):
		...
		self.meta = json.loads(get_property('coalition_playback_meta'))
		...
		def _scraperDialog():
			...
			while not self.progress_dialog.iscanceled() or monitor.abortRequested():
				try:
					...
					progress_bar = ((len(self.source_dict)-len_alive_threads)/len(self.source_dict))*100
					self.progress_dialog.update(format_line % (line1, line2, line3), progress_bar)
					sleep(self.sleep_time)
					if self.threads_completed:
						if len_alive_threads == 0 or percent >= 100: break
						if self.finish_early and self.sources and progress_bar >= 51: break
				except: pass
			return
		def _background():
			sleep(len(self.source_dict)*250)
			start_time = time.time()
			end_time = start_time + self.timeout
			while time.time() < end_time:
				alive_threads = [x for x in self.threads if x.is_alive()]
				len_alive_threads = len(alive_threads)
				sleep(self.sleep_time * max(len_alive_threads, 1))
				if len_alive_threads == 0 or self.sources: return
		self.threads = []
	...
	def process_filters(self):
		def _process(result_list, target):
			for item in result_list:
				threaded_object = Thread(target=target, args=(item,))
				threads_append(threaded_object)
				threaded_object.start()
		...
		try:
			...
			for item in debrid_hash_tuple:
				if item[0] in self.debrid_torrents:
					torrent_results += ([dict(i, **{'cache_provider':item[0]}) for i in torrent_sources if i['hash'] in cached_hashes[item[1]]])
					if self.display_uncached_torrents:
