...
from gears.control import existsPath, dataPath, makeFile, undesirablescacheFile, lang
...
class Undesirables():
	...
	def set_defaults(self):
		from gears.source_utils import UNDESIRABLES
...
def undesirablesSelect():
	from gears.control import multiselectDialog
	from gears.source_utils import UNDESIRABLES
...
def undesirablesUserRemove():
	from gears.control import multiselectDialog, notification
	undesirables_cache = Undesirables()
...
def undesirablesUserRemoveAll():
	from gears.control import yesnoDialog, notification
	undesirables_cache = Undesirables()
	user_undesirables = undesirables_cache.get_user_defined()
	if not user_undesirables: return notification(message=32084)
...
def undesirablesInput():
	from gears.control import dialog
	undesirables_cache = Undesirables()
