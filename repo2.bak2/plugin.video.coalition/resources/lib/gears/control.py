...
addonObject = addon('plugin.video.coalition') # addonObject = addon('script.module.gearsscrapers')
...
cacheFile = joinPath(dataPath, 'gearscache.db')
undesirablescacheFile = joinPath(dataPath, 'gearsundesirables.db')
...
#	try: settings_dict = jsloads(homeWindow.getProperty('gearsscrapers_settings'))
	try: settings_dict = jsloads(homeWindow.getProperty('coalition_settings'))
...
#		homeWindow.setProperty('gearsscrapers_settings', jsdumps(settings_dict))
		homeWindow.setProperty('coalition_settings', jsdumps(settings_dict))
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear gearsscrapers.log if setting to reverse has been changed
	if homeWindow.getProperty('gearsscrapers.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('gearsscrapers.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.gearsscrapers/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('gearsScrapers Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.gearsscrapers')
		xbmcvfs.mkdirs(addon_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = addon('script.module.gearsscrapers').getAddonInfo('version')
...
def clean_settings():
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
	addon_id = 'script.module.gearsscrapers'
	try:
		removed_settings = []
		active_settings = []
