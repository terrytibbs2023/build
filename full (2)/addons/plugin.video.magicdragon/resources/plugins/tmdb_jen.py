...
def run(url,lang,icon,fanart,plot,name):
    ...
    if 'year/movie' in url:
        url2='https://api.themoviedb.org/3/discover/movie?api_key=366726613e793048709ba27db15b7787&language=%s&sort_by=popularity.desc&include_adult=false&include_video=false&primary_release_year=%s&with_original_language=en&page=1'%(lang,url.split('year/movie/')[1])
        mode=14
        ...
        url2='https://api.themoviedb.org/3/discover/movie?api_key=366726613e793048709ba27db15b7787&with_companies={0}&language={1}&sort_by=popularity.desc&timezone=America%2FNew_York&include_null_first_air_dates=false&page=1'.format(company_id,lang)
        mode=14
    elif 'keyword' in url:
        split_url = url.split("/")
        if len(split_url) == 3:
        ...
        url2='http://api.themoviedb.org/3/discover/%s?with_keywords=%s&api_key=366726613e793048709ba27db15b7787&language=%s&page=1'%(s_type,keyword_id,lang)
        mode=14
    elif 'collection/' in url:
        split_url = url.split("/")
        id = split_url[-1]
        url2='http://api.themoviedb.org/3/%s?api_key=366726613e793048709ba27db15b7787&language=%s&page=1'%(url,lang)
        mode=179
    elif 'network' in url:
        split_url = url.split("/")
        if len(split_url) == 3:
        ...
        url2='https://api.themoviedb.org/3/discover/tv?api_key=366726613e793048709ba27db15b7787&with_networks={0}&language={1}&sort_by=popularity.desc&timezone=America%2FNew_York&include_null_first_air_dates=false&page=1'.format(network_id,lang)
        mode=14
    elif 'genre' in url:
        split_url = url.split("/")
        if len(split_url) == 3:
        ...
        if media == "movie":
            ...
            s_type='tv'
        url2='http://api.themoviedb.org/3/discover/%s?api_key=366726613e793048709ba27db15b7787&sort_by=popularity.desc&with_genres=%s&language=%s&page=1'%(s_type,genre_id,lang)
        mode=14
    elif 'list/' in url:
        id = url.split("/")[-1]
        url2='http://api.themoviedb.org/3/%s?api_key=366726613e793048709ba27db15b7787&language=%s&page=1'%(url,lang)
