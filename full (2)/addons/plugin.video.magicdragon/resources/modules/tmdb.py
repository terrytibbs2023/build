...
def adv_gen_window(url):
    ...
    class adv_gen_window(pyxbmct.AddonDialogWindow):
        ...
        def set_info_controls(self):
            ...
            url='http://api.themoviedb.org/3/genre/%s/list?api_key=366726613e793048709ba27db15b7787&language=%s&page=1'%(self.type,lang)
            x=get_html(url).json()
            self.all_g=[]
...
def get_html_g():
    try:
        url_g='https://api.themoviedb.org/3/genre/tv/list?api_key=366726613e793048709ba27db15b7787&language='+lang
        html_g_tv=get_html(url_g).json()
         
   
        url_g='https://api.themoviedb.org/3/genre/movie/list?api_key=366726613e793048709ba27db15b7787&language='+lang
           ...
           for data in result_html:
             ...
             if 'original_title' in data:
               ...
               if data['original_language']!='en':
                ...
                html2=get_html('http://api.themoviedb.org/3/movie/%s?api_key=366726613e793048709ba27db15b7787'%id).json()
               ...
               if data['original_language']!='en':
                    ...
                    html2=get_html('http://api.themoviedb.org/3/tv/%s?api_key=366726613e793048709ba27db15b7787'%id).json()
                    if 'name' in html2:
...
def get_movies(url,local=False,reco=0,global_s=False,return_results=False):
   ...
   if 'advance' in url:
        ...
        typee=url.split('_')[1]
        if typee=='movie':
            url='http://api.themoviedb.org/3/discover/%s?api_key=366726613e793048709ba27db15b7787&language=%s&sort_by=popularity.desc&primary_release_date.gte=%s-01-01&primary_release_date.lte=%s-12-31&with_genres=%s&page=1'%(typee,lang,start_y,end_y,','.join(all_g))
        else:
            url='http://api.themoviedb.org/3/discover/%s?api_key=366726613e793048709ba27db15b7787&language=%s&sort_by=popularity.desc&first_air_date.gte=%s-01-01&first_air_date.lte=%s-12-31&with_genres=%s&page=1'%(typee,lang,start_y,end_y,','.join(all_g))
   if url=='movie_years&page=1':
      ...
      if Addon.getSetting("dip_dialog")=='0':
          ...
          if ret!=-1:
              ...
              url='https://api.themoviedb.org/3/discover/movie?api_key=366726613e793048709ba27db15b7787&language=%s&sort_by=popularity.desc&include_adult=false&include_video=false&primary_release_year=%s&with_original_language=en&page=1'%(lang,all_years[ret])
        ...
        for items in all_years:
            ...
            url='https://api.themoviedb.org/3/discover/movie?api_key=366726613e793048709ba27db15b7787&language=%s&sort_by=popularity.desc&include_adult=false&include_video=false&primary_release_year=%s&with_original_language=en&page=1'%(lang,items)
            if 0:
   ...
   if url=='tv_years&page=1' and 'page=1' in url:
      if Addon.getSetting("dip_dialog")=='0':
          ret=ret = xbmcgui.Dialog().select("Choose", all_years)
          if ret!=-1:
            url='https://api.themoviedb.org/3/discover/tv?api_key=366726613e793048709ba27db15b7787&language=%s&sort_by=popularity.desc&first_air_date_year=%s&include_null_first_air_dates=false&with_original_language=en&page=1'%(lang,all_years[ret])
           
          else:
            sys.exit()
      else:
        for items in all_years:
            url='https://api.themoviedb.org/3/discover/tv?api_key=366726613e793048709ba27db15b7787&language=%s&sort_by=popularity.desc&first_air_date_year=%s&include_null_first_air_dates=false&with_original_language=en&page=1'%(lang,items)
...
def get_seasons(name,url,iconimage,fanart,description,data,original_title,id,heb_name):
   ...
   t = TVDB()
   if 'tvdb' not in id:
       url=domain_s+'api.themoviedb.org/3/tv/%s?api_key=366726613e793048709ba27db15b7787&language=%s&append_to_response=external_ids'%(id,lang)
...
def get_episode(name,url,iconimage,fanart,description,data,original_title,id,season,tvdb_id,show_original_year,heb_name):
   ...
   url=domain_s+'api.themoviedb.org/3/tv/%s/season/%s?api_key=366726613e793048709ba27db15b7787&language=%s&append_to_response=external_ids'%(id,season,lang)
   tmdbKey = '653bb8af90162bd98fc7ee32bcbbfb3d'
   html=get_html(url).json()
   #tmdb data
   ...
   if 'episodes'  in html:
      if len(html['episodes'])>0:
       ...
       if 'name' not in html['episodes'][0] or html['episodes'][0]['name']=='':
         url=domain_s+'api.themoviedb.org/3/tv/%s/season/%s?api_key=366726613e793048709ba27db15b7787&language=eng'%(id,season)
