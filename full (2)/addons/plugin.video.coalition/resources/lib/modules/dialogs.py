# -*- coding: utf-8 -*-
import json
from caches import clear_cache, refresh_cached_meta
...
def random_choice(tmdb_id, poster):
	...
	from modules.utils import manual_function_import # from modules import random_play
	function = manual_function_import('modules.episode_tools', choice) # exec('random_play.%s(tmdb_id)' % choice)
	function(tmdb_id)

def trakt_manager_choice(params):
	if not get_setting('trakt_user', ''): return notification(32760, 3500)
	icon = translate_path('special://home/addons/plugin.video.coalition/resources/media/trakt.png')
...
def set_quality_choice(quality_setting):
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'coalition', 'enumerate': 'false', 'multi_choice': 'true', 'multi_line': 'false', 'preselect': preselect}
...
def extras_lists_choice():
	screenshots_directory = 'special://home/addons/plugin.video.coalition/resources/media/screenshots/extras/%s'
...
def folder_scraper_manager_choice(folder_info=None):
	def _get_property(setting_id):
		return get_property('coalition_%s' % setting_id) or get_setting(setting_id)
	def _set_property(setting_id, setting_value):
		set_property('coalition_%s' % setting_id, setting_value)
	...
	def _exit_save_settings():
		for folder_no in range(1,6):
			...
			_clear_property('coalition_%s' % name_setting % folder_no)
			_clear_property('coalition_%s' % movie_dir_setting % folder_no)
			_clear_property('coalition_%s' % tvshow_dir_setting % folder_no)
	...
	def _set_folder_path():
		if _get_property(setting) not in ('', 'None'):
			...
			kwargs = {'items': json.dumps(list_items), 'heading': 'coalition', 'enumerate': 'false', 'multi_choice': 'false', 'multi_line': 'false'}
			...
			if action == 1:
				...
				folder = dialog.browse(0, 'coalition', '')
...
def results_layout_choice():
	screenshots_directory = 'special://home/addons/plugin.video.coalition/resources/media/screenshots/results/%s'
	xml_choices = [
...
def color_choice(msg_dialog='coalition', no_color=False):
...
def favorites_choice(params):
	icon = translate_path('special://home/addons/plugin.video.coalition/resources/media/favourites.png')
	if params.get('cache'):
		...
		if any(i['tmdb_id'] == tmdb_id for i in current_favourites): action, text = favourites.remove_from_favourites, '%s coalition %s?' % (ls(32603), ls(32453))
		else: action, text = favourites.add_to_favourites, '%s coalition %s?' % (ls(32602), ls(32453))
		if not confirm_dialog(heading='coalition - %s' % title, text=text, top_space=True): return
		action(media_type, tmdb_id, title)

def external_scrapers_choice():
#	icon = translate_path('special://home/addons/script.module.gearsscrapers/icon.png')
	icon = translate_path('special://home/addons/plugin.video.coalition/resources/lib/gears/media/icon.png')
	...
	kwargs = {'items': json.dumps(list_items), 'heading': 'coalition', 'enumerate': 'false', 'multi_choice': 'false', 'multi_line': 'true'}
...
def options_menu(params, meta=None):
	...
	results_sorting_status = get_setting('results.sort_order_display').replace('$ADDON[plugin.video.coalition 32582]', ls(32582))
...
#	listing += [('%s %s %s' % (open_str, ls(32522), settings_str), '', 'open_scraper_settings')]
	...
	listing += [('%s %s %s' % (open_str, ls(32036), settings_str), '', 'open_coalition_settings')]
...
#	elif choice == 'open_scraper_settings': return execute_builtin('Addon.OpenSettings(script.module.gearsscrapers)')
	elif choice == 'open_coalition_settings': return kodi_utils.open_settings('0.0')
...
def build_navigate_to_page(params):
	use_alphabet = settings.nav_jump_use_alphabet()
	icon = translate_path('special://home/addons/plugin.video.coalition/resources/media/item_jump.png')
