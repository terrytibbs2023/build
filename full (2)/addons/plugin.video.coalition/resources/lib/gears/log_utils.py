...
import inspect
from gears.control import transPath, setting as getSetting, lang, joinPath, existsPath
...
DEBUGPREFIX = '[COLOR red][ gearsSCRAPERS %s ][/COLOR]'
...
def log(msg, caller=None, level=LOGINFO):
	...
	try:
		...
		if debug_location == '1':
			log_file = joinPath(LOGPATH, 'gearsscrapers.log')
		...
		traceback.print_exc()
		import xbmc
		xbmc.log('[ script.module.gearsonscrapers ] log_utils.log() Logging Failure: %s' % (e), LOGERROR)
...
#			addon = 'script.module.gearsscrapers'
			addon = 'plugin.video.coalition'
		...
		import xbmc
		xbmc.log('[ script.module.gearsonscrapers ] log_utils.error() Logging Failure: %s' % (e), LOGERROR)

def clear_logFile():
	cleared = False
	try:
		from gears.control import yesnoDialog
		...
		import xbmc
		xbmc.log('[ script.module.gearsonscrapers ] log_utils.clear_logFile() Failure: %s' % (e), LOGERROR)
		cleared = False
	return cleared

def view_LogFile(name):
	try:
		from gears.textviewer import TextViewerXML
		from gears.control import addonPath
		log_file = joinPath(LOGPATH, '%s.log' % name.lower())
		if not existsPath(log_file):
			from gears.control import notification
...
def upload_LogFile():
	...
	try:
		import requests
		from gears.control import addonVersion, selectDialog
		f = open(log_file, 'r', encoding='utf-8', errors='ignore')
		text = f.read()
		f.close()
		UserAgent = 'gearsScrpaers %s' % addonVersion()
		...
		if 'key' in response.json():
			...
			log('gearsScrapers log file uploaded to: %s' % result)
			...
			if 'Copy url To Clipboard' in list[select][0]:
				from gears.source_utils import copy2clip
				copy2clip(list[select - 1][1])
		elif 'message' in response.json():
			notification(message='gearsScrapers Log upload failed: %s' % str(response.json()['message']))
			log('gearsScrapers Log upload failed: %s' % str(response.json()['message']), level=LOGERROR)
		else:
			notification(message='gearsScrapers Log upload failed')
			log('gearsScrapers Log upload failed: %s' % response.text, level=LOGERROR)
	except:
		error('gearsScrapers log upload failed')
