...
class RealDebridAPI:
	...
	def auth(self):
		...
		kodi_utils.progressDialog.update(100,
			line % (ls(32517), ls(32700) % response.get('verification_url'), ls(32701) % response.get('user_code'))
		)
	...
	def user_cloud(self):
		string = 'coalition_rd_user_cloud'
		url = 'torrents'
		return cache_object(self._get, string, url, False, 0.5)

	def downloads(self):
		string = 'coalition_rd_downloads'
		url = 'downloads'
		return cache_object(self._get, string, url, False, 0.5)

	def user_cloud_info(self, file_id):
		string = 'coalition_rd_user_cloud_info_%s' % file_id
	...
	def get_hosts(self):
		string = 'coalition_rd_valid_hosts'
		url = 'hosts/domains'
	...
	def clear_cache(self):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""SELECT data FROM maincache WHERE id=?""", ('coalition_rd_user_cloud',))
				try:
				...
				if not user_cloud_success:
					dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('coalition_rd_user_cloud',))
					clear_property("coalition_rd_user_cloud")
					for i in user_cloud_info_caches:
						dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('coalition_rd_user_cloud_info_%s' % i,))
						clear_property("coalition_rd_user_cloud_info_%s" % i)
					dbcon.commit()
					user_cloud_success = True
			except: user_cloud_success = False
			# DOWNLOAD LINKS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('coalition_rd_downloads',))
				clear_property("coalition_rd_downloads")
				dbcon.commit()
				download_links_success = True
			except: download_links_success = False
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('coalition_rd_valid_hosts',))
				clear_property('coalition_rd_valid_hosts')
				dbcon.commit()
				dbcon.close()
