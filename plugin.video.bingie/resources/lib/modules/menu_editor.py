...
class MenuEditor:
	...
	def _path_browser(self, label='', file='plugin://plugin.video.bingie?mode=navigator.main&full_list=true', thumbnail=''):
		kodi_utils.show_busy_dialog()
		results = kodi_utils.jsonrpc_get_directory(file)
		kodi_utils.hide_busy_dialog()
		list_items, function_items = [], []
		if file != 'plugin://plugin.video.bingie?mode=navigator.main&full_list=true':
	...
	def _get_menu_item(self, path):
		return dict(parse_qsl(path.replace('plugin://plugin.video.bingie/?','')))
	...
	def _get_icon_var(self, icon_path):
		try:
			...
			icon_value = icon_value.split(kodi_utils.translatePath('special://home/addons/plugin.video.bingie/resources/media/icons/'))[1]
