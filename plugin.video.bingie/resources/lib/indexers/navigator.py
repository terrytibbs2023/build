...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'Bingie')
	...
	def tools(self):
		...
		if get_setting('bingie.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('bingie.use_viewtypes', 'true') == 'true' and not get_setting('bingie.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		...
		bingie_clogpath = k.translate_path('special://home/addons/plugin.video.bingie/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': bingie_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def networks(self):
		...
		networks = sorted(function(), key=lambda k: k['name'])
		for i in networks: self.add({'mode': mode, 'action': action, 'key_id': i['id'], 'name': i['name']}, i['name'], i['icon'], original_image=True)
		self.end_directory()
	...
	def genres(self):
		...
		genres = function()
		for i in genres: self.add({'mode': mode, 'action': action, 'key_id': i['id'], 'name': i['name']}, i['name'], i['icon'])
		self.end_directory()
	...
	def exit_media_menu(self):
		params = k.get_property('bingie.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.bingie/resources/text/tips'
