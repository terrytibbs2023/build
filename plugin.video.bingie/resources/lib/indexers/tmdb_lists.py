...
def get_tmdb_lists(params):
	...
	try:
		data = get_all_tmdb_lists(get_setting('bingie.tmdblist.list_sort'))
		if data:
			if shuffle_lists:
				returning_to_list = 'build_tmdb_lists_contents' in kodi_utils.folder_path()
				if returning_to_list:
					try: data = json.loads(kodi_utils.get_property('bingie.tmdb.lists.order'))
					except: pass
				else:
					shuffle(data)
					kodi_utils.set_property('bingie.tmdb.lists.order', json.dumps(data))
			else:
				kodi_utils.clear_property('bingie.tmdb.lists.order')
...
def adjust_tmdb_list_properties(params):
	sort_order_dict = {'0': 'Title', '1': 'Date Added (asc)', '2': 'Date Added (desc)', '3': 'Shuffle'}
	...
	try:
		if sort_order:
			if sort_order in ('', '0', 'None'):
				contents = sort_for_article(contents, 'name')
