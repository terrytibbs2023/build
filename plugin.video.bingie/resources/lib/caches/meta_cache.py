...
class MetaCache:
	...
	def get_memory_cache(self, media_type, id_type, media_id, current_time):
		try:
			prop_string = 'bingie.%s_%s_%s' % (media_type, id_type, media_id)
	...
	def get_memory_cache_season(self, prop_string, current_time):
		try:
			cachedata = eval(get_property('bingie.meta_season_%s' % prop_string))
			if cachedata[0] > current_time: result = cachedata[1]
		except: result = None
		return result
	...
	def set_memory_cache(self, media_type, id_type, meta, expires, media_id):
		try:
			cachedata, prop_string = (expires, meta), 'bingie.%s_%s_%s' % (media_type, id_type, media_id)
	...
	def set_memory_cache_season(self, prop_string, meta, expires):
		try:
			...
			set_property('bingie.meta_season_%s' % prop_string, repr(cachedata))
		except: pass

	def delete_memory_cache(self, media_type, id_type, media_id):
		try: clear_property('bingie.%s_%s_%s' % (media_type, id_type, media_id))
		except: pass

	def delete_memory_cache_season(self, prop_string):
		try: clear_property('bingie.meta_season_%s' % prop_string)
		except: pass
