...
def window_manager(obj):
	def close():
		obj.close()
		kodi_utils.clear_property('bingie.window_loaded')
		kodi_utils.clear_property('bingie.window_stack')

	def monitor():
		timer = 0
		while not kodi_utils.get_property('bingie.window_loaded') == 'true' and timer <= 5:
			kodi_utils.sleep(50)
			timer += 0.05
	...
	def get_stack():
		try: window_stack = json.loads(kodi_utils.get_property('bingie.window_stack'))
		except: window_stack = []
		return window_stack

	def add_to_stack(params):
		window_stack.append(params)
		kodi_utils.set_property('bingie.window_stack', json.dumps(window_stack))
...
class BaseDialog(xbmcgui.WindowXMLDialog):
	...
	def set_home_property(self, prop, value):
		kodi_utils.set_property('bingie.%s' % prop, value)

	def get_home_property(self, prop):
		return kodi_utils.get_property('bingie.%s' % prop)

	def clear_home_property(self, prop):
		return kodi_utils.clear_property('bingie.%s' % prop)
...
class FontUtils:
	def execute_custom_fonts(self):
		...
		skin_files = kodi_utils.list_dirs(kodi_utils.translate_path('special://home/addons/plugin.video.bingie/resources/skins/Default/1080i/'))[1]
		for item in skin_files:
			self.replace_font(item, replacement_values)
		kodi_utils.set_property('bingie.current_skin', self.current_skin)
		kodi_utils.set_property('bingie.current_font', self.current_font)
	...
	def skin_change_check(self):
		...
		if self.current_skin != kodi_utils.get_property('bingie.current_skin') or self.current_font != kodi_utils.get_property('bingie.current_font'): return True
		return False

	def match_font(self, size, bold, fallback):
		font_tag = 'Bingie_%s%s' % (size, '_BOLD' if bold else '')
	...
	def replace_font(self, window, replacement_values):
		file = kodi_utils.translate_path('special://home/addons/plugin.video.bingie/resources/skins/Default/1080i/' + window)
