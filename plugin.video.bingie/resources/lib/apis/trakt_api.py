...
def call_trakt(path, params={}, data=None, is_delete=False, with_auth=True, method=None, pagination=False, page_no=1):
	def send_query():
		resp = None
		if with_auth:
			while kodi_utils.get_property('bingie.trakt_refreshing_token') == 'true':
				kodi_utils.logger('refreshing trakt token', '')
				kodi_utils.sleep(250)
			try: expires_at = float(get_setting('bingie.trakt.expires'))
			except: expires_at = 0.0
			if time.time() > expires_at: trakt_refresh_token()
			token = get_setting('bingie.trakt.token')
...
def trakt_refresh_token():
	try:
		...
		CLIENT_SECRET = settings.trakt_secret()
		if CLIENT_SECRET in (None, 'empty_setting', ''): return no_secret_key()
		kodi_utils.set_property('bingie.trakt_refreshing_token', 'true')
		data = {        
			'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET, 'redirect_uri': 'urn:ietf:wg:oauth:2.0:oob',
			'grant_type': 'refresh_token', 'refresh_token': get_setting('bingie.trakt.refresh')}
	...
	kodi_utils.clear_property('bingie.trakt_refreshing_token')
...
def trakt_revoke_authentication(dummy=''):
	...
	CLIENT_SECRET = settings.trakt_secret()
	if CLIENT_SECRET in (None, 'empty_setting', ''): return no_secret_key()
	data = {'token': get_setting('bingie.trakt.token'), 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
...
def trakt_calendar_days(recently_aired, current_date):
	if recently_aired: start, finish = (current_date - timedelta(days=14)).strftime('%Y-%m-%d'), '14'
		...
		previous_days = int(get_setting('bingie.trakt.calendar_previous_days', '0'))
		future_days = int(get_setting('bingie.trakt.calendar_future_days', '7'))
...
def trakt_sync_activities(force_update=False):
	...
	def refresh_token_check():
		...
		sync_interval = int(get_setting('bingie.trakt.sync_interval', '60')) * 60
		try: expires_at = float(get_setting('bingie.trakt.expires'))
		except: expires_at = 0.0
	...
	def _check_daily_expiry():
		return int(time.time()) >= int(get_setting('bingie.trakt.next_daily_clear', '0'))
