...
from bingie.parser import try_int, try_float, dict_to_list, get_params, IterProps
from tmdbbingiehelper.lib.api.mapping import UPDATE_BASEKEY, _ItemMapper, get_empty_item
from tmdbbingiehelper.lib.addon.plugin import get_mpaa_prefix, get_language, convert_type, get_setting, get_localized
from tmdbbingiehelper.lib.addon.consts import ITER_PROPS_MAX
...
class ItemMapperMethods:
    ...
    def tmdb_imagepath(self):
        from tmdbbingiehelper.lib.api.tmdb.images import TMDbImagePath
    ...
    def get_release_types(v, iso_country):
        from tmdbbingiehelper.lib.addon.tmdate import is_future_timestamp
        infoproperties = {}
        info_release_types = []
    ...
    def get_extra_art(self, v):
        ...
        if landscape:
            landscape = sorted(landscape, key=lambda i: (0 if i.get('iso_639_1') == self.iso_language else 1,-i.get('vote_average', 0)))
            artwork['landscape'] = self.tmdb_imagepath.get_imagepath_thumbs(landscape[0].get('file_path'))

        clearlogo = [i for i in v.get('logos', []) if i.get('iso_639_1') and i.get('file_path', '')[-4:] != '.svg']
        if clearlogo:
            clearlogo = sorted(clearlogo, key=lambda i: (0 if i.get('iso_639_1') == self.iso_language else 1,-i.get('vote_average', 0)))
        ...
        extrafanart = [i for i in v.get('backdrops', []) if not i.get('iso_639_1') and i.get('aspect_ratio') == 1.778]
        if extrafanart:
            extrafanart = sorted(extrafanart, key=lambda i: -i.get('vote_average', 0))        
            for idx, item in enumerate(extrafanart[:ITER_PROPS_MAX], start=1):
                if item.get('file_path'):
                    artwork[f'fanart{idx}'] = self.tmdb_imagepath.get_imagepath_fanart(item['file_path'])
    ...
    def get_episode_to_air(self, v, name):
        from tmdbbingiehelper.lib.addon.tmdate import format_date_obj, convert_timestamp, get_days_to_air
        i = v or {}
    ...
    def get_crew_properties(self, v):
        ...
        for i in v:
            ...
            if i.get('job') == 'Director':
                department_map['Directing']['x'] += 1
                infoproperties.update(self.set_crew_properties(i, department_map['Directing']['x'], 'director'))
        x = 0
        for i in v:
            if not i.get('name'):
                continue
...
class ItemMapper(_ItemMapper, ItemMapperMethods):
    ...
    def finalise(self, item, tmdb_type):
        ...
        def finalise_person():
            from tmdbbingiehelper.lib.addon.tmdate import age_difference
