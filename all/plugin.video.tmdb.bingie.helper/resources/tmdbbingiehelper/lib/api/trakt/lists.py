...
from tmdbbingiehelper.lib.addon.plugin import convert_type, PLUGINPATH, get_plugin_category, get_localized, get_setting, encode_url
...
class ListBasic(ContainerDirectory):
    def get_items(
        ...
        from tmdbbingiehelper.lib.addon.consts import TRAKT_BASIC_LISTS
...
class ListSync(ContainerDirectory):
    def get_list_items(self, sync_type, item_type, page=1, limit=None, sort_by=None, sort_how=None, params=None, next_page=True, filters=None, tmdb_id=None, **kwargs):
        from tmdbbingiehelper.lib.api.trakt.sync.itemlist import ItemListSyncDataFactory
        from tmdbbingiehelper.lib.items.itemlist import ItemListPagination
    ...
    def get_items(self, info, tmdb_type, page=None, **kwargs):
        from tmdbbingiehelper.lib.addon.consts import TRAKT_SYNC_LISTS
...
class ListDropped(ListSync):
    def get_items(self, info, tmdb_type, page=None, **kwargs):
        """ Trakt hidden dropped shows """
        if tmdb_type != 'tv':
            return
        self.tmdb_cache_only = False
        ...
        self.plugin_category = f'{convert_type(tmdb_type, "plural")} {get_localized(32541)}'
...
class ListComments(ContainerDirectory):
    def get_items(self, info, tmdb_type, tmdb_id, sort=None, **kwargs):
        """ Get a mix of watchlisted and inprogress """
        from tmdbbingiehelper.lib.api.mapping import get_empty_item
        ...
        items = self.trakt_api.get_request_sc(f'{trakt_type}s', slug, 'comments', sort, limit=limit)
...
class ListBecauseYouWatched(ListSync):
    def get_items(self, info, tmdb_type, page=None, **kwargs):
        ...
        from tmdbbingiehelper.lib.api.tmdb.lists import ListBasic as TMDbListBasic
...
class ListCalendar(ContainerDirectory):
    def _get_calendar_items(self, info, startdate, days, page=None, kodi_db=None, endpoint=None, user=True, **kwargs):
        from tmdbbingiehelper.lib.addon.tmdate import get_calendar_name
...
class ListLibraryCalendar(ListCalendar):
    def get_items(self, **kwargs):
        from tmdbbingiehelper.lib.api.kodi.rpc import get_kodi_library
...
class ListLists(ContainerDirectory):
    def get_items(self, info, page=None, **kwargs):
        ...
        from tmdbbingiehelper.lib.addon.consts import TRAKT_LIST_OF_LISTS
