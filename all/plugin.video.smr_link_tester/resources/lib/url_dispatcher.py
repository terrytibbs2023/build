...
class URL_Dispatcher:
        ...
        * If there are no arguments at all, just "mode" can be specified
        ...
        def decorator(f):
            if mode in self.func_registry:
                message = 'Error: {0} already registered as {1}'.format(f, mode)
    ...
    def dispatch(self, mode, queries):
        ...
        if mode not in self.func_registry:
            message = 'Error: Attempt to invoke unregistered mode |{0}|'.format(mode)
            raise Exception(message)
        ...
        if self.args_registry[mode]:
            ...
            for arg in self.args_registry[mode]:
                ...
                if arg in queries:
                    ...
                    message = 'Error: mode |{0}| requested argument |{1}| but it was not provided.'.format(mode, arg)
                    raise Exception(message)
    ...
    def showmodes(self):
        ...
        for mode in sorted(self.func_registry, key=lambda x: int(x)):
            ...
            line = 'Mode {0} Registered - {1} args: {2} kwargs: {3}'.format(mode, value, args, kwargs)
