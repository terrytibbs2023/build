...
def play_using(play_using, mode='play', **kwargs):
    ...
    def _update_from_listitem(dictionary):
        ...
        if url[-5:] == '.strm':
            url = read_file(url)
        params = {}
        if url.startswith('plugin://plugin.video.tmdb.bingie.helper/?'):
            params = parse_paramstring(url.replace('plugin://plugin.video.tmdb.bingie.helper/?', ''))
...
def update_players():
    ...
    downloader = Downloader(
        extract_to='special://profile/addon_data/plugin.video.tmdb.bingie.helper/players',
