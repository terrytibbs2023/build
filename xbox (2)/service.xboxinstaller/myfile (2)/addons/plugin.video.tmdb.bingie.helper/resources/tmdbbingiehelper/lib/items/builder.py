import re
from tmdbbingiehelper.lib.items.artselect import _ArtworkSelector
from tmdbbingiehelper.lib.addon.plugin import get_setting
from tmdbbingiehelper.lib.items.listitem import ListItem
from tmdbbingiehelper.lib.files.bcache import BasicCache, BasicCacheService
from tmdbbingiehelper.lib.api.tmdb.api import TMDb
from tmdbbingiehelper.lib.api.fanarttv.api import FanartTV
from tmdbbingiehelper.lib.addon.tmdate import set_timestamp, get_timestamp
from tmdbbingiehelper.lib.addon.consts import IMAGEPATH_QUALITY_POSTER, IMAGEPATH_QUALITY_FANART, IMAGEPATH_QUALITY_THUMBS, IMAGEPATH_QUALITY_CLOGOS, IMAGEPATH_ALL, ARTWORK_BLACKLIST
from tmdbbingiehelper.lib.addon.thread import ParallelThread
from tmdbbingiehelper.lib.addon.logger import TimerList, kodi_log

FTV_SECOND_PREF = True if get_setting('fanarttv_secondpref') and get_setting('fanarttv_lookup') else False
...
class ItemBuilder(_ArtworkSelector):
    ...
    def get_tmdb_item(
        ...
        with TimerList(self.timer_lists, 'item_tmdb', log_threshold=0.05, logging=self.log_timers) as tl:
            ...
            item['listitem']['art'] = {}
            item['fanarttv_enfallback'] = get_setting('fanarttv_enfallback')
            item['fanarttv_lookup'] = get_setting('fanarttv_lookup')
            item['fanarttv_secondpref'] = get_setting('fanarttv_secondpref')
            item['artwork_quality'] = ARTWORK_QUALITY
        return item
    ...
    def get_item(self, tmdb_type, tmdb_id, season=None, episode=None, cache_refresh=False):
        ...
        if item and get_timestamp(item['expires']):  # Our item hasn't expired
            # Force refresh if FTV-related settings have changed
            current_enfallback = get_setting('fanarttv_enfallback')
            cached_enfallback = item.get('fanarttv_enfallback')
            current_ftvlookup = get_setting('fanarttv_lookup')
            cached_ftvlookup = item.get('fanarttv_lookup')
            current_secondpref = get_setting('fanarttv_secondpref')
            cached_secondpref = item.get('fanarttv_secondpref')
            current_artwork_quality = get_setting('artwork_quality', 'int')
            cached_artwork_quality = item.get('artwork_quality', ARTWORK_QUALITY)
            if (cached_enfallback != current_enfallback or
                cached_ftvlookup != current_ftvlookup or
                cached_secondpref != current_secondpref or
                cached_artwork_quality != current_artwork_quality):
                return self.get_item(tmdb_type, tmdb_id, season, episode, cache_refresh=True)
    ...
    def get_item_artwork(self, artwork, art_dict=None, is_season=False):
        ...
        if FTV_SECOND_PREF:
            ...
            if get_setting('fanarttv_lookup'):
