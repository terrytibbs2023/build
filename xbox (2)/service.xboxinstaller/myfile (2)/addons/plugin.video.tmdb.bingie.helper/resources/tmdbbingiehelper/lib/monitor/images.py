...
from tmdbbingiehelper.lib.addon.plugin import get_infolabel, get_setting, get_condvisibility, ADDONDATA
from bingie.window import WindowPropertySetter
from bingie.parser import try_int, try_float
from tmdbbingiehelper.lib.files.futils import make_path
...
class ImageFunctions(SafeThread, WindowPropertySetter):
    ...
    blur_size = try_int(get_infolabel('Skin.String(TMDbBingieHelper.Blur.Size)')) or 480
    crop_size = (800, 310)
    radius = try_int(get_infolabel('Skin.String(TMDbBingieHelper.Blur.Radius)')) or 40
    ...
    def __init__(self, method=None, artwork=None, is_thread=True, prefix='ListItem'):
        ...
        if method == 'blur':
            self.func = self.blur
            self.save_path = make_path(self.save_path.format('blur_v1'))
            self.save_prop = f'{prefix}.BlurImage'
            self.save_orig = True
        elif method == 'crop':
            self.func = self.crop
            self.save_path = make_path(self.save_path.format('crop_v1'))
            self.save_prop = f'{prefix}.CropImage'
            self.save_orig = True
        elif method == 'desaturate':
            self.func = self.desaturate
            self.save_path = make_path(self.save_path.format('desaturate_v1'))
            self.save_prop = f'{prefix}.DesaturateImage'
            self.save_orig = True
        elif method == 'colors':
            self.func = self.colors
            self.save_path = make_path(self.save_path.format('colors_v1'))
    ...
    def get_color_lumsat(self, r, g, b):
        hls_tuple = colorsys.rgb_to_hls(r / 255.0, g / 255.0, b / 255.0)
        hue = hls_tuple[0]
        lum = try_float(get_infolabel('Skin.String(TMDbBingieHelper.Colors.Luminance)')) or hls_tuple[1]
        sat = try_float(get_infolabel('Skin.String(TMDbBingieHelper.Colors.Saturation)')) or hls_tuple[2]
...
class ImageManipulations(WindowPropertySetter):
    ...
    def get_image_manipulations(self, use_winprops=False, built_artwork=None, allow_list=('crop', 'blur', 'desaturate', 'colors', )):
        ...
        _manipulations = (
            {'method': 'crop',
                'active': lambda: get_condvisibility("Skin.HasSetting(TMDbBingieHelper.EnableCrop)"),
                'images': lambda: self.get_artwork(
                    source=CROPIMAGE_SOURCE,
                    build_fallback=True,
                    built_artwork=built_artwork)},
            {'method': 'blur',
                'active': lambda: get_condvisibility("Skin.HasSetting(TMDbBingieHelper.EnableBlur)"),
                'images': lambda: self.get_artwork(
            ...
            {'method': 'desaturate',
                'active': lambda: get_condvisibility("Skin.HasSetting(TMDbBingieHelper.EnableDesaturate)"),
                'images': lambda: self.get_artwork(
            ...
            {'method': 'colors',
                'active': lambda: get_condvisibility("Skin.HasSetting(TMDbBingieHelper.EnableColors)"),
                'images': lambda: self.get_artwork(
