...
def playsetresolved2(url, name, iconimage, setresolved=True):
    ...
    if url and '$doregex[' in str(url):
        ...
        xbmc.log(f"Found regex patterns: {regex_patterns}", xbmc.LOGDEBUG)
        
        for regex_name in regex_patterns:
            xbmc.log(f"Processing regex: {regex_name}", xbmc.LOGDEBUG)
            
            regex_result = None
            
            if regex_name in regexs_data:
                regex_def = regexs_data[regex_name]
                xbmc.log(f"Found regex definition for {regex_name}: {regex_def}", xbmc.LOGDEBUG)
                
                regex_result = execute_regex(regex_def)
                if regex_result:
                    xbmc.log(f"Regex execution result: {regex_result}", xbmc.LOGDEBUG)
                else:
                    xbmc.log(f"Regex execution failed or returned empty", xbmc.LOGDEBUG)
            
            if regex_result:
                pattern_to_replace = f'$doregex[{regex_name}]'
                
                # Check if the result looks like a hex string (hex characters only)
                hex_pattern = re.compile(r'^[0-9a-fA-F]+$')
                if hex_pattern.match(regex_result):
                    xbmc.log(f"Detected hex string, attempting to decode", xbmc.LOGDEBUG)
                    decoded_result = hex_decode(regex_result)
                    xbmc.log(f"Hex decoded result: {decoded_result}", xbmc.LOGDEBUG)
                    url = url.replace(pattern_to_replace, decoded_result)
                else:
                    url = url.replace(pattern_to_replace, regex_result)
                
                xbmc.log(f"Replaced {pattern_to_replace} with regex result", xbmc.LOGDEBUG)
                xbmc.log(f"URL after replacement: {url}", xbmc.LOGDEBUG)
            else:
                xbmc.log(f"Could not execute regex: {regex_name}", xbmc.LOGERROR)
    
    # STEP 1.5: CHECK AND DECODE HEX STRINGS IN ENTIRE URL
    xbmc.log(f"Checking for hex encoding in URL: {url[:100]}...", xbmc.LOGDEBUG)
    
    # Check if the entire URL (before pipe) looks like a hex string
    url_str = str(url)
    if '|' in url_str:
    ...
    if hex_pattern.match(url_before_pipe):
        xbmc.log(f"URL appears to be hex encoded, attempting to decode", xbmc.LOGDEBUG)
        
        try:
            decoded_url = hex_decode(url_before_pipe)
            xbmc.log(f"Successfully hex decoded: {decoded_url}", xbmc.LOGDEBUG)
            
            # Reconstruct URL with decoded part
            if headers_part:
                url = decoded_url + headers_part
            else:
                url = decoded_url
        except Exception as e:
            xbmc.log(f"Failed to hex decode: {str(e)}", xbmc.LOGERROR)
    else:
        # Also check for specific hex patterns that might be part of a larger URL
        hex_matches = re.findall(r'([0-9a-fA-F]{20,})', url_before_pipe)
        for hex_match in hex_matches:
            if len(hex_match) > 30:  # Only decode substantial hex strings
                xbmc.log(f"Found hex substring in URL: {hex_match[:50]}...", xbmc.LOGDEBUG)
                try:
                    decoded_part = hex_decode(hex_match)
                    if '://' in decoded_part:  # Check if it looks like a URL after decoding
                        url_before_pipe = url_before_pipe.replace(hex_match, decoded_part)
                        if headers_part:
                            url = url_before_pipe + headers_part
                        else:
                            url = url_before_pipe
                        xbmc.log(f"Replaced hex substring with decoded URL", xbmc.LOGDEBUG)
                        break
                except Exception as e:
                    xbmc.log(f"Failed to decode hex substring: {str(e)}", xbmc.LOGDEBUG)
    
    # STEP 2: EXTRACT HEADERS FIRST
    headers = {}
    headers_part = ""
            ...
            if '&' in headers_str:
                ...
                for pair in header_pairs:
                    if '=' in pair:
                        try:
                            ...
                            value = value.strip()
                            headers[key] = value
                            xbmc.log(f"Parsed header: {key} = {value[:50]}...", xbmc.LOGDEBUG)
                        except Exception as e:
                ...
                if '=' in headers_str:
                    try:
                        ...
                        value = value.strip()
                        headers[key] = value
                        xbmc.log(f"Parsed single header: {key} = {value[:50]}...", xbmc.LOGDEBUG)
                    except Exception as e:
                        xbmc.log(f"Error parsing single header: {str(e)}", xbmc.LOGERROR)
            
            headers_part = '|' + headers_str
            url = url_without_headers
    
    # STEP 3: CHECK FOR JSON DATA WITH CLEARKEY DRM
    ...
    if ('manifestUrl' in str(url) and 'clearkey' in str(url)) or ('"manifestUrl"' in str(url) and '"clearkey"' in str(url)):
        xbmc.log(f"Found possible JSON DRM data in URL", xbmc.LOGDEBUG)
        
        try:
            json_match = re.search(r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}', str(url))
            if json_match:
                json_str = json_match.group(0)
                xbmc.log(f"Extracted JSON string: {json_str[:200]}...", xbmc.LOGDEBUG)
                
                data = json.loads(json_str)
                manifest_url = data.get('manifestUrl', '')
                clearkey_str = data.get('clearkey', '{}')
                
                xbmc.log(f"Manifest URL from JSON: {manifest_url}", xbmc.LOGDEBUG)
                xbmc.log(f"ClearKey string from JSON: {clearkey_str}", xbmc.LOGDEBUG)
                
                if manifest_url and clearkey_str:
                    clearkey_str = clearkey_str.replace('\\"', '"').replace('\\u0026', '&')
                    clearkey_data = json.loads(clearkey_str)
                    
                    xbmc.log(f"Parsed ClearKey data: {clearkey_data}", xbmc.LOGDEBUG)
                    ...
                    if clearkey_data and isinstance(clearkey_data, dict):
                        ...
                        xbmc.log(f"Set ClearKey DRM: {license_key}", xbmc.LOGDEBUG)
                        xbmc.log(f"New URL after JSON parsing: {url}", xbmc.LOGDEBUG)
                        json_processed = True
                        
        except json.JSONDecodeError as e:
            xbmc.log(f"Failed to parse JSON: {str(e)}", xbmc.LOGERROR)
            try:
                manifest_match = re.search(r'"manifestUrl":"([^"]+)"', url)
                clearkey_match = re.search(r'"clearkey":"([^"]+)"', url)
                
                if manifest_match and clearkey_match:
                    manifest_url = manifest_match.group(1)
                    clearkey_str = clearkey_match.group(1)
                    
                    key_match = re.search(r'\{"([^"]+)":"([^"]+)"\}', clearkey_str)
                    if key_match:
                        ...
                        xbmc.log(f"Set ClearKey DRM (alternative method): {license_key}", xbmc.LOGDEBUG)
                        json_processed = True
            except Exception as e2:
                xbmc.log(f"Alternative JSON parsing also failed: {str(e2)}", xbmc.LOGERROR)
        except Exception as e:
            xbmc.log(f"Error processing JSON DRM data: {str(e)}", xbmc.LOGERROR)
    
    # STEP 4: BASE64 DECODING (if no JSON DRM was found)
    if not json_processed and 'aHR0cHM6Ly9'.lower() in str(url).lower() and not ('manifestUrl' in str(url) or '.mpd' in str(url).lower()):
        ...
        if not headers_part and '|' in str(url):
            url_parts = str(url).split('|')
            if len(url_parts) > 1:
                headers_part = '|' + '|'.join(url_parts[1:])
                xbmc.log(f"Headers part to preserve: {headers_part}", xbmc.LOGDEBUG)
        
        url_lower = str(url).lower()
        base64_start = url_lower.find('ahr0chm6ly9')
        
        if base64_start != -1:
            xbmc.log(f"Base64 starts at position: {base64_start}", xbmc.LOGDEBUG)
            ...
            xbmc.log(f"Encoded string to decode: {encoded_str}", xbmc.LOGDEBUG)
            
            decoded_url = base64_decode(encoded_str)
            if decoded_url:
                xbmc.log(f"Successfully decoded URL: {decoded_url}", xbmc.LOGDEBUG)
                url = decoded_url + headers_part if headers_part else decoded_url
            else:
                xbmc.log(f"Base64 decoding failed", xbmc.LOGERROR)
        else:
            xbmc.log(f"Could not find Base64 start position", xbmc.LOGERROR)
    else:
        xbmc.log(f"No Base64 prefix found or URL is DASH manifest", xbmc.LOGDEBUG)
    
    # STEP 5: FINAL HEADERS PROCESSING
    # If URL still has headers pipe, parse them again
    if '|' in str(url) and not headers:
        ...
        if len(url_parts) > 1:
            headers_str = '|'.join(url_parts[1:])
            xbmc.log(f"Headers string to parse: {headers_str}", xbmc.LOGDEBUG)
            
            header_pairs = headers_str.split('&')
            xbmc.log(f"Header pairs: {header_pairs}", xbmc.LOGDEBUG)
            ...
            for pair in header_pairs:
                if '|' in pair:
                    ...
                    for sub_pair in sub_pairs:
                        if '=' in sub_pair:
                            try:
                                ...
                                value = value.strip()
                                headers[key] = value
                                xbmc.log(f"Parsed header (from | split): {key} = {value[:50]}...", xbmc.LOGDEBUG)
                            except Exception as e:
                    ...
                    xbmc.log(f"Skipping invalid header pair: {pair}", xbmc.LOGDEBUG)
    
    # Build headers string for InputStream Adaptive
    header_string = ''
    if headers:
    ...
    # STEP 6: SET UP INPUTSTREAM ADAPTIVE PROPERTIES
    ...
    if '.m3u8' in url_lower:
        ...
        xbmc.log(f"Detected HLS stream", xbmc.LOGDEBUG)
    elif '.mpd' in url_lower or 'format=mpd' in url_lower:
        liz.setMimeType('application/dash+xml')
        liz.setContentLookup(False)
        liz.setProperty('inputstream.adaptive.manifest_type', 'mpd')
        xbmc.log(f"Detected DASH stream", xbmc.LOGDEBUG)
    
    # Ensure URL doesn't have headers pipe at the end
    if '|' in str(url):
        url = str(url).split('|')[0]
    ...
    # Final debug logging
    xbmc.log(f"Final URL: {url}", xbmc.LOGDEBUG)
