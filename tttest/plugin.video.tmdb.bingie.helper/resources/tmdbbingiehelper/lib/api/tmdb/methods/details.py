...
def get_tmdb_multisearch_validfy(query=None, validfy=True, scrub=True):
    ...
    from tmdbbingiehelper.lib.files.futils import validify_filename
...
def get_tvshow_nextaired(self, tmdb_id):
    ...
    from tmdbbingiehelper.lib.addon.tmdate import format_date
    from tmdbbingiehelper.lib.addon.plugin import get_infolabel
    from tmdbbingiehelper.lib.api.tmdb.mapping import ItemMapperMethods
    ...
    def _get_formatted(ip):
        df = get_infolabel('Skin.String(TMDbBingieHelper.Date.Format)') or '%d %b %Y'
