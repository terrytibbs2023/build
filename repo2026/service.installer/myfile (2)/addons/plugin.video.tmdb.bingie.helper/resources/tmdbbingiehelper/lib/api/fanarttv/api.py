from tmdbbingiehelper.lib.addon.plugin import get_language, get_setting
from bingie.parser import try_int, del_empty_keys
from tmdbbingiehelper.lib.addon.consts import CACHE_EXTENDED, ITER_PROPS_MAX
from tmdbbingiehelper.lib.api.request import RequestAPI
from tmdbbingiehelper.lib.api.api_keys.fanarttv import API_KEY, CLIENT_KEY
...
class FanartTV(RequestAPI):
    ...
    def get_all_artwork(self, ftv_id, ftv_type, season=None, artlist_type=None, season_type=None):
        ...
        def get_best_artwork(key, get_lang=True):
            ...
            try:
                ...
                if isinstance(get_lang, str):
                    return
                if get_lang and get_setting('fanarttv_enfallback'):
                    return get_best_artwork(key, 'en')
                return
        ...
        if artlist_type:
            all_items = get_artwork(artlist_type, get_list=True, get_lang='all') or []
            langs = {self.language, '00', None, ''}
            if get_setting('fanarttv_enfallback'):
                langs.add('en')
            artwork_data = [i for i in all_items if i.get('lang') in langs]
            return artwork_data
