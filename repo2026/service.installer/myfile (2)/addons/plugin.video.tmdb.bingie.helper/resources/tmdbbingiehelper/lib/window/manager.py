...
def get_listitem(path):
    try:
        ...
        if base == 'plugin://plugin.video.tmdb.bingie.helper/':
...
class WindowManager(_EventLoop):
    ...
    def add_origin(self, tmdb_type, tmdb_id):
        if not tmdb_type or not tmdb_id:
            return
        path = f'plugin://plugin.video.tmdb.bingie.helper/?info=details&tmdb_type={tmdb_type}&tmdb_id={tmdb_id}'
    ...
    def make_query(self, query, tmdb_type, separator=' / '):
        ...
        if not tmdb_id:
            ...
            return
        return f'plugin://plugin.video.tmdb.bingie.helper/?info=details&tmdb_type={tmdb_type}&tmdb_id={tmdb_id}'
