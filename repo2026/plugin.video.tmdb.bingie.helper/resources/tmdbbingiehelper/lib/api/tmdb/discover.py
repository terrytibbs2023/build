...
from bingie.parser import try_int, merge_two_dicts, split_items, find_dict_list_index
from tmdbbingiehelper.lib.items.container import ContainerDirectory
from tmdbbingiehelper.lib.addon.plugin import ADDONPATH, PLUGINPATH, convert_type, get_localized, encode_url
from tmdbbingiehelper.lib.addon.tmdate import get_datetime_now, get_timedelta
from bingie.window import get_property
from tmdbbingiehelper.lib.files.hcache import set_search_history, get_search_history
...
class ListDiscover(ContainerDirectory):
    def get_items(self, tmdb_type, **kwargs):
        ...
        limit_str = kwargs.pop('limit', None)
        try:
            limit = int(limit_str) if limit_str is not None else 0
        except (ValueError, TypeError):
            limit = 0
        items = self.tmdb_api.get_discover_list(tmdb_type, **_translate_discover_params(tmdb_type, kwargs))
        if limit > 0 and items:
            items = items[:limit]
