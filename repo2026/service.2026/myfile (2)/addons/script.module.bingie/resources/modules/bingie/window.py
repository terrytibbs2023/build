import xbmc
import xbmcgui
from bingie.parser import try_type, try_int
...
class WindowChecker():
    ...
    def get_window_property(self, key, is_type=None, is_home=False):
        ...
        if not window:
            return
        return try_type(window.getProperty(f'TMDbBingieHelper.{key}'), is_type or str)
...
def get_property(name, set_property=None, clear_property=False, window_id=None, prefix=None, is_type=None):
    if prefix != -1:
        prefix = prefix or 'TMDbBingieHelper'
        name = f'{prefix}.{name}'
...
class WindowProperty():
    def __init__(self, *args, prefix='TMDbBingieHelper'):
...
class WindowPropertySetter():
    ...
    def get_property(self, name, set_property=None, clear_property=False, is_type=None, prefix='TMDbBingieHelper'):
