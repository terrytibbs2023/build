...
FULL_SRC_PATH="$SRC_DIR/$PKG_PATH"
mkdir -p "$FULL_SRC_PATH"

cat > "$FULL_SRC_PATH/$MAIN_ACTIVITY.java" <<EOF
package $PACKAGE_NAME;
...
javac -source 1.8 -target 1.8 -bootclasspath "$ANDROID_JAR" \
    -d "$BUILD_DIR/bin" "$FULL_SRC_PATH/$MAIN_ACTIVITY.java"

CLASS_FILE="$BUILD_DIR/bin/$MAIN_ACTIVITY.class"
if [ ! -f "$CLASS_FILE" ]; then
  echo "❌ Compilation failed: $MAIN_ACTIVITY.class not found"
  echo "🔍 Check if the source file exists at: $FULL_SRC_PATH/$MAIN_ACTIVITY.java"
  echo "🔍 Check if the package declaration matches the folder structure"
  exit 1
fi
