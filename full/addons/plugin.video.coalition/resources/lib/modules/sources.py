...
from gears import sources as gears_sources
...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		self._update_meta()
		self._search_info()
		set_property('coalition_playback_meta', json.dumps(self.meta))
		self.get_sources()
	...
	def get_sources(self):
		...
		self.meta['scrape_time'] = time.time()-start_time
	...
	def activate_external_providers(self):
		if not self.debrid_torrent_enabled and not self.debrid_hoster_enabled:
			...
			external_providers = gears_sources(ret_all=self.disabled_ignored)
	...
	def play_execute_background(self, results):
		...
		set_property('coalition_background_url', background_url)
	...
	def furkPacks(self, name, file_id, highlight=None, download=False):
		...
		default_furk_icon = translate_path('special://home/addons/plugin.video.coalition/resources/media/furk.png')
		...
		return coalitionPlayer().run(link, 'video')
	...
	def debridPacks(self, debrid_provider, name, magnet_url, info_hash, highlight=None, download=False):
		...
		default_debrid_icon = translate_path('special://home/addons/plugin.video.coalition/resources/media/%s' % icon)
	...
	def play_file(self, results, source={}, autoplay=False, background=False):
		...
		try:
			...
			return coalitionPlayer().run(self.url)
		except: pass
			...
			if 'cache_provider' in item:
				...
				if 'Uncached' in cache_provider:
					if cache_item:
						...
						return coalitionPlayer().run(url)
					else:
						url = 'uncached'
						return url
					return None
	...
	def resolve_internal_sources(self, scrape_provider, item_id, url_dl, direct_debrid_link=False):
		...
		try:
			if scrape_provider == 'furk':
				...
				meta = json.loads(get_property('coalition_playback_meta'))
