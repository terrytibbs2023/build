...
class FurkAPI:
	...
	def search(self, query, expiration=48):
		try:
			...
			string = 'coalition_FURK_SEARCH_%s' % url
	...
	def direct_search(self, query):
		try:
			...
			string = 'coalition_FURK_SEARCH_DIRECT_%s' % url
	...
	def t_files(self, file_id):
		try:
			...
			string = 'coalition_%s_%s' % ('FURK_T_FILE', file_id)
...
def clear_media_results_database():
	...
	dbcur.execute("SELECT id FROM maincache WHERE id LIKE 'coalition_FURK_SEARCH_%'")
	try:
		furk_results = [str(i[0]) for i in dbcur.fetchall()]
		if not furk_results: return 'success'
		dbcur.execute("DELETE FROM maincache WHERE id LIKE 'coalition_FURK_SEARCH_%'")
