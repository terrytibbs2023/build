...
current_skin_prop, current_font_prop = 'bingie.current_skin', 'bingie.current_font'
addon_skins_folder = 'special://home/addons/plugin.video.bingie/resources/skins/Default/1080i/'
...
def window_manager(obj):
	def close():
		obj.close()
		clear_property('bingie.window_loaded')
		clear_property('bingie.window_stack')

	def monitor():
		timer = 0
		while not get_property('bingie.window_loaded') == 'true' and timer <= 5:
			sleep(50)
			timer += 0.05
		hide_busy_dialog()
		obj.close()
	...
	def get_stack():
		try: window_stack = json.loads(get_property('bingie.window_stack'))
		except: window_stack = []
		return window_stack

	def add_to_stack(params):
		window_stack.append(params)
		set_property('bingie.window_stack', json.dumps(window_stack))
...
class BaseDialog(window_xml_dialog):
	...
	def set_home_property(self, prop, value):
		set_property('bingie.%s' % prop, value)

	def get_home_property(self, prop):
		return get_property('bingie.%s' % prop)

	def clear_home_property(self, prop):
		return clear_property('bingie.%s' % prop)
