...
levels =['../../../..', '../../..', '../..', '..']
status_property_string = 'bingie.download_status.%s'
...
def download_threads_manager(multi_downloads, image):
	...
	for item in multi_downloads:
		...
		set_property('bingie.active_queued_downloads', json.dumps(remaining_downloads))
	clear_property('bingie.active_queued_downloads')
...
class Downloader:
	...
	def get_active_downloads(self):
		return json.loads(get_property('bingie.active_downloads') or '[]')
	...
	def add_active_download(self):
		...
		set_property('bingie.active_downloads', json.dumps(active_downloads))
	...
	def remove_active_download(self):
		...
		if active_downloads: set_property('bingie.active_downloads', json.dumps(active_downloads))
		else: self.clear_active_downloads()

	def clear_active_downloads(self):
		clear_property('bingie.active_downloads')

	def set_percent_property(self, percent):
		set_property('bingie.%s' % self.final_name, str(percent))
