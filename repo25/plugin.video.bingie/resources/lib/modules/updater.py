...
destination_check = translate_path('special://home/addons/plugin.video.bingie/')
changelog_location = translate_path('special://home/addons/plugin.video.bingie/resources/text/changelog.txt')
downloads_icon = get_icon('downloads')
addon_dir = 'plugin.video.bingie'
zipfile_name = 'plugin.video.bingie-%s.zip'
heading_str = 'Bingie Updater'
error_str = 'Error'
notification_occuring_str = 'Bingie Update Occuring'
notification_available_str = 'Bingie Update Available'
notification_updating_str = 'Bingie Performing Update'
notification_rollback_str = 'Bingie Performing Rollback'
...
success_str = '[CR]Success.[CR]Bingie updated to version [B]%s[/B]'
rollback_heading_str = 'Choose Rollback Version'
success_rollback_str = '[CR]Success.[CR]Bingie rolled back to version [B]%s[/B]'
confirm_rollback_str = 'Are you sure?[CR]Version [B]%s[/B] will overwrite your current installed version.' \
						'[CR]Bingie will set your update action to [B]OFF[/B] if rollback is successful'
...
no_changes_str = 'You are running the current version of Bingie.[CR][CR]There is no new version changelog to view.'
...
def rollback_check():
	...
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results.json() if 'plugin.video.bingie' in i['name'] \
