...
int_window_prop = 'bingie.internal_results.%s'
scraper_timeout = 25
...
class Sources():
		...
		if self.active_external or self.background:
			if self.active_external:
				if any((i[0] == 'comet' for i in self.external_providers)):
					...
					debrid_token = get_setting('bingie.%s' % debrid_token_dict[debrid_service])
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('bingie.%s.display_name' % i), i, source_folders_directory(self.media_type, i)) for i in folder_scrapers]
