...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		self.folders_ignore_filters = get_setting('bingie.results.folders_ignore_filters', 'false') == 'true'
		self.filter_size_method = int(get_setting('bingie.results.filter_size_method', '0'))
		...
		self.include_unknown_size = get_setting('bingie.results.size_unknown', 'false') == 'true'
	...
	def filter_results(self, results):
		...
		if self.filter_size_method:
			min_size = string_to_float(get_setting('bingie.results.%s_size_min' % self.media_type, '0'), '0') / 1000
			...
			if self.filter_size_method == 1:
				...
				max_size = string_to_float(get_setting('bingie.results.%s_size_max' % self.media_type, '10000'), '10000') / 1000
	...
	def special_filter(self, results, file_type):
		...
		if key == '[B]HEVC[/B]' and enable_setting == 0:
			hevc_max_quality = self._get_quality_rank(get_setting('bingie.filter.hevc.%s' % ('max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('bingie.%s.display_name' % i), i, settings.source_folders_directory(self.media_type, i))
	...
	def _process_internal_results(self):
		for i in self.internal_scrapers:
			win_property = kodi_utils.get_property('bingie.internal_results.%s' % i)
			if win_property in ('checked', '', None): continue
			try: sources = json.loads(win_property)
			except: continue
			kodi_utils.set_property('bingie.internal_results.%s' % i, 'checked')
	...
	def _clear_properties(self):
		...
		for item in def_internal: kodi_utils.clear_property('bingie.internal_results.%s' % item)
		if self.active_folders:
			for item in self.folder_info: kodi_utils.clear_property('bingie.internal_results.%s' % item[0])
