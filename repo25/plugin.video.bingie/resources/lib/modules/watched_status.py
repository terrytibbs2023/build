...
progress_db_string, indicators_dict = 'bingie_hidden_progress_items', {0: 'watched_db', 1: 'trakt_db'}
...
def clear_local_bookmarks():
	try:
		...
		file_ids = dbcon.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.bingie%'").fetchall()
