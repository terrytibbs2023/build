...
single_ep_list = ('episode.progress', 'episode.recently_watched', 'episode.next_trakt', 'episode.next_bingie', 'episode.trakt_recently_aired', 'episode.trakt_calendar')
...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.bingie').getAddonInfo(info)

def addon_version():
	return get_property('bingie.addon_version') or addon_info('version')

def addon_path():
	return get_property('bingie.addon_path') or addon_info('path')

def addon_profile():
	return get_property('bingie.addon_profile') or translatePath(addon_info('profile'))

def addon_icon():
	return get_property('bingie.addon_icon') or addon_info('icon')

def addon_fanart():
	return get_property('bingie.addon_fanart') or addon_info('fanart')

def get_icon(image_name):
	return img_url % getattr(icons, image_name, 'I1JJhji')

def get_addon_fanart():
	return get_property('bingie.default_addon_fanart') or addon_fanart()

def build_url(url_params):
	return 'plugin://plugin.video.bingie/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('bingie.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('bingie.%s' % view_type) or None
...
def addon(addon_id='plugin.video.bingie'):
...
def run_addon(addon='plugin.video.bingie', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'bingie' not in get_infolabel('Container.PluginName')
...
def disable_enable_addon(addon_name='plugin.video.bingie'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.bingie'):
	import time
...
def external_scraper_settings():
	try:
		external = get_property('bingie.external_scraper.module')
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('Bingie', line1, icon or addon_icon(), time)
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('bingie.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('bingie.playback.volumecheck_percent') or '50'), 100)
...
def toggle_language_invoker():
	...
	close_all_dialog()
	addon_xml = translate_path('special://home/addons/plugin.video.bingie/addon.xml')
