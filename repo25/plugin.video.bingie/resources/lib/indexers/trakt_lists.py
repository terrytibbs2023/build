	...
	try:
		...
		if shuffle:
			...
			if returning_to_list:
				try: lists = json.loads(get_property('bingie.trakt.lists.order'))
				except: pass
			else:
				random.shuffle(lists)
				set_property('bingie.trakt.lists.order', json.dumps(lists))
			sort_method = 'none'
		else:
			clear_property('bingie.trakt.lists.order')
			sort_method = 'label'
...
def build_trakt_list(params):
	...
	try:
		...
		if page_no == 1 and not is_external: set_property('bingie.exit_params', folder_path())
