...
class Movies:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external: set_property('bingie.exit_params', folder_path())
	...
	def build_movie_content(self, _position, _id):
		try:
			...
			set_properties({'bingie.extras_params': extras_params, 'bingie.options_params': options_params,
							'belongs_to_collection': belongs_to_movieset, 'bingie.more_like_this_params': more_like_this_params})
