...
class RandomLists():
	def __init__(self, params):
		...
		self.remake_widgets = self.is_external and get_property('bingie.refresh_widgets') == 'true'
	...
	def make_directory(self, next_page_params={}):
		...
		if self.is_external:
			if self.folder_name: set_property('bingie.%s' % self.folder_name, self.category_name)
			else: set_property('bingie.%s' % self.base_list_name, self.category_name)
...
def random_shortcut_folders(folder_name, random_results):
	...
	remake_widgets = is_external and get_property('bingie.refresh_widgets') == 'true'
	...
	if is_external: set_property('bingie.%s' % folder_name, list_name)
