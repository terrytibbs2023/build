...
def set_quality_choice(params):
	...
	fl = ['SD', '720p', '1080p', '4K']
	try: preselect = [fl.index(i) for i in get_setting('bingie.%s' % quality_setting).split(', ')]
	except: preselect = []
...
def set_language_filter_choice(params):
	...
	try: preselect = [fl.index(i) for i in get_setting('bingie.%s' % filter_setting_id).split(', ')]
	except: preselect = []
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or get_icon('bingie')
