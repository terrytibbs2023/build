...
class TVShows:
	...
	def fetch_list(self):
		...
		try:
			...
			if page_no == 1 and not self.is_external:
				...
				if not any([x in folderpath for x in internal_nav_check]): set_property('bingie.exit_params', folderpath)
	...
	def build_tvshow_content(self, _position, _id):
		try:
			...
			set_properties({'bingie.extras_params': extras_params, 'bingie.options_params': options_params, 'bingie.more_like_this_params': more_like_this_params})
