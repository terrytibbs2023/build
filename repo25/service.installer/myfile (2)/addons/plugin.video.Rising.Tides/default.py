...
def playsetresolved2(url, name, iconimage, setresolved=True):
    ...
    # Create the ListItem (liz) before setting its properties
    liz = xbmcgui.ListItem(label=name)
    liz.setArt({'icon': iconimage, 'thumb': iconimage})

    # Check if the URL contains a Base64 encoded string
    base64_prefix = 'ahr0chm6ly9'
    if base64_prefix in url:
        base64_start = url.find(base64_prefix)
        if base64_start != -1:
            encoded_str = url[base64_start:]
            base64_end = encoded_str.find('|')
            if base64_end != -1:
                encoded_str = encoded_str[:base64_end]
                
            # Decode the Base64 string
            decoded_url = base64_decode(encoded_str)
            if decoded_url:
                xbmc.log(f"Decoded URL: {decoded_url}", xbmc.LOGDEBUG)
                url = decoded_url
            else:
                xbmc.log(f"Base64 decoding failed for: {encoded_str}", xbmc.LOGERROR)

    # Extract query parameters from the URL
    parsed_url = urllib_parse.urlparse(url)
    query_params = urllib_parse.parse_qs(parsed_url.query)
    xbmc.log(f"Extracted parameters: {query_params}", xbmc.LOGDEBUG)

    # Handle additional ListItem properties based on URL
    if '$$lic' in url:
        url, lic = url.split('$$lic=', 1)
