...
def tmdb_list_order_choice(params):
	choices = [('Title', '0'),
	('Date Created (asc)', '1'), ('Date Created (desc)', '2'),
	('Recently Updated (asc)', '3'), ('Recently Updated (desc)', '4'),
	('Item Count (asc)', '5'), ('Item Count (desc)', '6'),
	('Average Rating (asc)', '7'), ('Average Rating (desc)', '8'),
	('Total Runtime (asc)', '9'), ('Total Runtime (desc)', '10'),
	('Total Revenue (asc)', '11'), ('Total Revenue (desc)', '12')]
	list_items = [{'line1': i[0]} for i in choices]
	kwargs = {'items': json.dumps(list_items), 'narrow_window': 'true'}
	choice = kodi_utils.select_dialog(choices, **kwargs)
	if choice == None: return
	set_setting('tmdblist.list_sort_name', choice[0])
	set_setting('tmdblist.list_sort', choice[1])

def language_invoker_choice(params):
	from xml.dom.minidom import parse as mdParse
	kodi_utils.close_all_dialog()
	addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.bingie/addon.xml')
...
def addon_icon_choice(params):
	...
	icons_path = 'special://home/addons/plugin.video.bingie/resources/media/addon_icons'
...
def personallists_manager_choice(params):
	...
	if action == 'add_new':
		list_name, sort_order = make_new_personal_list({'external_creation': 'true'})
...
def tmdblists_manager_choice(params):
	...
	# all_lists.sort(key=lambda k: (k['name']))
	choices = []
	if not all_lists: action = 'add_new'
	else:
...
def set_quality_choice(params):
	...
	fl = ['SD', '720p', '1080p', '4K']
	q_setting = get_setting('bingie.%s' % quality_setting).split(', ')
	...
	if not orig_button_dict:
		for _type in ('movie', 'tvshow'):
			...
			for item in range(10, 18):
				...
				try:
					button_action = get_setting('bingie.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
				except:
					set_setting(setting_id.replace('bingie.', ''), default_setting_values(setting_id.replace('bingie.', ''))['setting_default'])
					button_action = get_setting('bingie.%s' % setting_id)
					button_label = extras_button_label_values[_type][button_action]
...
def set_language_filter_choice(params):
	...
	set_filter = get_setting('bingie.%s' % filter_setting_id).split(', ')
...
def enable_scrapers_choice(params={}):
	icon = params.get('icon', None) or kodi_utils.get_icon('bingie')
...
def scraper_color_choice(params):
	...
	current_setting, original_highlight = get_setting('bingie.%s' % setting), default_setting_values(setting)['setting_default']
