...
def include_exclude_filters():
	return {
'hevc': 'HEVC', '3d': '3D', 'hdr': 'HDR', 'dv': 'D/VISION', 'd/vision': 'D/VISION', 'av1': 'AV1',
'enhanced_upscaled': 'AI ENHANCED/UPSCALED', 'ai enhanced/upscaled': 'AI ENHANCED/UPSCALED', 'hybrid': 'HYBRID'}
...
def internal_results(provider, sources):
	set_property('bingie.internal_results.%s' % provider, json.dumps(sources))
...
def get_external_cache_status(data, debrid, token, services):
	import re
	import base64
	import requests
	...
	def _busy_check(service):
		...
		prop = 'bingie.%s_cache_check' % service
		while get_property(prop) == 'true':
			...
			if count >= 9000: break
		if pause_needed:
			# logger('pause_needed %s - %s' % (service, debrid), count)
			sleep(1000)
	def _process(service):
		data = []
		if service == 'mediafusion':
			_busy_check(service)
			set_property('bingie.%s_cache_check' % service, 'true')
		...
		try:
			...
			# logger('DATA %s - %s' % (service, debrid), len(data))
		except: pass
		if service in ('mediafusion', 'comet'): set_property('bingie.%s_cache_check' % service, 'false')
		results.extend(data)
	try:
		results = []
		token = get_setting('bingie.%s' % token)
		movie_link = '/stream/movie/%s.json'
		tv_link = '/stream/series/%s:%s:%s.json'
		imdb_id = data['imdb']
		threads = [Thread(target=_process, args=(item,)) for item in services]
