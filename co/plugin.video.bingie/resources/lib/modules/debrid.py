...
def RD_check(data, hash_list, cached_hashes, external_cache_check):
	...
	if unchecked_hashes:
		results = get_external_cache_status(data, 'realdebrid', 'rd.token', ['torrentio', 'mediafusion'])
		if results:
...
def AD_check(data, hash_list, cached_hashes, external_cache_check):
	...
	if unchecked_hashes:
		results = get_external_cache_status(data, 'alldebrid', 'ad.token', ['comet', 'mediafusion'])
		if results:
			cached_append = cached_hashes.append
			process_list = []
			process_append = process_list.append
