...
class PremiumizeAPI:
	def __init__(self):
		self.token = get_setting('bingie.pm.token', 'empty_setting')
	...
	def headers(self):
		return {'User-Agent': 'Bingie for Kodi', 'Authorization': 'Bearer %s' % self.token}

	def _get(self, url, data={}):
		if self.token in ('empty_setting', ''): return None
		headers = {'User-Agent': 'Bingie for Kodi', 'Authorization': 'Bearer %s' % self.token}
