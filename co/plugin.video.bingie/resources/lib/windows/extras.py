...
class Extras(BaseDialog):
	...
	def make_ratings(self, win_prop=4000):
		...
		for prop, _id in (('metascore', 1), ('tomatometer', 2), ('tomatousermeter', 3), ('imdb', 4), ('tmdb', 5)):
			...
			self.setProperty('%s_rating' % prop, 'true'), self.set_label(win_prop + _id, rating), self.set_image(win_prop + 100 + _id, 'bingie_flags/ratings/%s' % icon)
	...
	def assign_buttons(self):
		setting_id_base = 'bingie.extras.%s.button' % self.media_type
		...
		for item in butts:
			...
			try:
				...
				self.restore_setting_default({'setting_id': setting_id.replace('bingie.', ''), 'silent': 'true'})
				button_action = self.get_setting(setting_id)
				button_label = self.button_label_values[self.media_type][button_action]
